...
- `upcomingDeadlines` includes active reminder rows from the recent-overdue window through future dates so FE-001 can truthfully display both `D+N` and `D-N` states,
- recent-overdue window is the server date-only `today` minus 7 calendar days; older overdue rows are excluded from the dashboard queue but remain available from the VaultItem deadline list,
- ordering is: overdue rows first with the most recently missed date first, then `today`, then future rows nearest first; total limit 10,
...
- no D-day string is generated by the API; FE-001 derives localized `D-3`, `D-DAY`, `D+2` display from the returned date-only value using the shared date helper.
...
5. Dashboard projection ordering/limits/recent-overdue/empty result.
...
- dashboard returns only real caller-owned data, recent overdue/future dates, and honest empty arrays,
