...
const validEnv = {
  ...
  BOUQUET_REDIRECT_URI: "http://localhost:3000/auth/bouquet/callback",
...
describe("getAuthConfig", () => {
  it("parses the valid development contract", () => {
    ...
    expect(config.appBaseUrl.toString()).toBe("http://localhost:3000/");
    expect(config.bouquetBaseUrl.toString()).toBe("http://localhost:8080/");
    expect(config.bouquetClientId).toBe("evidence-vault-local");
    expect(config.bouquetRedirectUri.toString()).toBe("http://localhost:3000/auth/bouquet/callback");
  ...
  it.each([
    ["app", { ...validEnv, NODE_ENV: "production", APP_BASE_URL: "http://app.example.com" }],
    ["provider", { ...validEnv, NODE_ENV: "production", BOUQUET_BASE_URL: "http://id.example.com" }],
    ["callback", { ...validEnv, NODE_ENV: "production", BOUQUET_REDIRECT_URI: "http://app.example.com/auth/bouquet/callback" }],
